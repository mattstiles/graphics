...
queue()
    .defer(d3.json, "data/json/election2012normal.json")
...
function ready(error, president, provinces) {
 ...
 // Create tooltip
    ...
    .offset([-10, 0])
    .html(function(d) {
        return "<strong>Place: </strong>" + [d.properties.name_eng] +  ", " + [d.properties.province] + "<br><strong>Votes: </strong>" + [d.properties.votes].toLocaleString(0) + "<br><strong>Park Vote Share: </strong>" + [d.properties.parkgeun_1].toLocaleString(0) + "%";
      })
 ...
 // municipalities (251 polygons: gu, gun, si)
   ...
   chartElement.append("g")
    .selectAll("path")
        .data(topojson.feature(president, president.objects.election2012normal).features)
    .enter().append("path")
        .attr("d", path)
