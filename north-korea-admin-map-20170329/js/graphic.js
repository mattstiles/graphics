...
function ready(error, nkcounty, nkprovince, asia) {
  ...
  // Create tooltip
    ...
    .offset([-10, 0])
    .html(function(d) {
        return "<strong>County: </strong>" + d.properties.NAME_2;
      })
...
// rivers
   ...
   // chartElement.append("g")
   //  .selectAll("path")
   //      .data(topojson.feature(nkwater, nkwater.objects.PRK_water_lines_dcw).features)
   //  .enter().append("path")
   //      .attr("d", path)
   //      // .on('mouseover', tooltip.show)
   //      // .on('mouseout', tooltip.hide)
   //      .attr("class", function(d, i) {
   //          return 'rivers ' + classify(d.properties.F_CODE_DES);
   //          });
...
//    chartElement.append("g")
//     .selectAll("path")
//         .data(topojson.feature(nkwater, nkwater.objects.PRK_water_areas_dcw).features)
//     .enter().append("path")
//         .attr("d", path)
//         // .on('mouseover', tooltip.show)
//         // .on('mouseout', tooltip.hide)
//         .attr("class", function(d, i) {
//             return 'lakes ' + classify(d.properties.F_CODE_DES);
//             });
