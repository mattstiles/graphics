...
var render = function(containerWidth) {
    ...
    dataSeries.forEach(function(v,k) {
        ...
        var showAxisLabels = k === 0 || k === 5 || k === 10 || k === 15 || k === 20 || k === 25 || k === 30 || k === 35 || k === 40 || k === 45;
...
var renderLineChart = function(config) {
    ...
    var usaaverage = d3.time.format('%m/%d/%Y').parse('01/01/1991')
            


var ranges = chartElement.append('g')
    .attr('class', 'ranges');
...
/*
        ...
        var propertyLabel = d3.time.format('%Y').parse('1842'),
            begin = d3.time.format('%Y').parse('1976'),
            end = d3.time.format('%Y').parse('2016')
        ...
        highlight_g.append('rect')
            ...
            .attr('transform', 'translate(' + xScale(begin) + ',0)');

    var usaaveragebetter = d3.time.format('%Y').parse('1977')
    var usaaverageworse = d3.time.format('%Y').parse('1977')


        // Render USA average highlight

        highlight_g.append('text')
            .attr('class', 'highlight-label label-usaaverageworse')
            .text(' ')
            .attr('text-anchor', 'begin')
            .attr('x', xScale(usaaverageworse))
            .attr('y', yScale(7))
            .html(LABELS['annotation_usaaverageworse']);

            highlight_g.append('text')
            .attr('class', 'highlight-label label-usaaveragebetter')
            .text(' ')
            .attr('text-anchor', 'begin')
            .attr('x', xScale(usaaveragebetter))
            .attr('y', yScale(-8))
            .html(LABELS['annotation_usaaveragebetter']);
