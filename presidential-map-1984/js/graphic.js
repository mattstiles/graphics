...
var renderStateGridMap = function(config) {
    ...
    _.each(config['data'], function(state) {
        if (state['category1984'] != null) {
            categories.push(state['category1984']);
        }
    });
    ...
    var colorScale = d3.scale.ordinal()
        .domain(categories)
        .range([COLORS['democrat3'], COLORS['republican3']]);
    ...
    _.each(config['data'], function(state) {
        if (state['category1984'] !== null) {
            ...
            chartElement.select('.' + stateClass)
                ...
                .attr('fill', colorScale(state['category1984']));
        }
    });
    ...
    chartElement.append('g')
        ...
        .enter().append('text')
            ...
            .attr('class', function(d) {
                return d['category1984'] !== null ? 'label label-active' : 'label';
            })
            .attr('x', function(d) {
