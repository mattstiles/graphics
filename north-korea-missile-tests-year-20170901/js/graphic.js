...
kimera_dates.forEach(function(d) {
...
var renderStackedColumnChart = function(config) {
    ...
    var colorScale = d3.scale.ordinal()
        ...
        .range([ COLORS['teal3'], COLORS['orange3'], '#e6e6e6' ]);
    ...
    var kimera = chartElement.append('g')
     .attr('class', 'kimera')
     .selectAll('rect')
     .data(kimera_dates)
     .enter()
         .append('rect')
             .attr('x', function(d) {
                 return xScale(d['nbegin']);
             })
             .attr('width', function(d) {
                 return xScale(d['nend']) - xScale(d['nbegin'] - 1);
             })
             .attr('y', 0)
    ...
    chartElement.append('text')
        ...
        .attr('x', function(){
            var dates = kimera_dates[0];
            if (isMobile) {
                return xScale(dates['nbegin']) + ((xScale(dates['nend']) - xScale(dates['nbegin'])) - 20);
            } else {
                return xScale(dates['nbegin']) + ((xScale(dates['nend']) - xScale(dates['nbegin'])) - 46);
            }
        })
