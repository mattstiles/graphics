...
var renderLineChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d, i) {
            if (isMobile) {
                return fmtAbbrMonth(d);
            } else {
                return fmtFullMonth(d);
            }
        });
    ...
    var allstar_label = d3.time.format('%m/%d/%Y').parse('07/08/16');
        allstar_begin = d3.time.format('%m/%d/%Y').parse('07/10/16');
        allstar_end = d3.time.format('%m/%d/%Y').parse('07/14/16');
        zeroLabel = d3.time.format('%m/%d/%Y').parse('07/30/16');  
    ...
    highlight_g.append('text')
        ...
        .attr('x', xScale(allstar_label))
        .attr('y', yScale(26))
        .html(LABELS['annotation_label-allstar'])

        highlight_g.append('text')
        .attr('class', 'highlight-label label-zero')
        .text(' ')
        .attr('text-anchor', 'end')
        .attr('x', xScale(zeroLabel))
        .attr('y', yScale(-2))
        .html(LABELS['annotation_label-zero'])
