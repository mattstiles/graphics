...
function ready(error, us, health, healthcongress) {
  ...
  //Create tooltip
    ...
    .offset([-10, 0])
    .html(function(d) {
        return "<strong>Member: </strong>" + memberName[d.properties.id] + "<br><strong>Party-State: </strong> " + partyName[d.properties.id] + "-" + stateName[d.properties.id] + "<br><strong>District: </strong>" + districtName[d.properties.id]  + "<br><strong>Strongly Opposed: </strong>" + rateById[d.properties.id].toLocaleString(0) + "%" + "<br><strong>Trump 2016 Vote: </strong>" + Math.abs([trumpVote[d.properties.id]] ).toLocaleString(0) + "%";
      })
